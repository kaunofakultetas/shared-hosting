...
| `/api/updatecaddyconfig` | POST | Update users Caddy configuration |
| `/api/updateportforwarderconfig` | POST | Update portforwarder configuration |
...
### 3.6 Users Portforwarder (`hosting-users-portforwarder`)

**Purpose**: Raw TCP entry point for user applications that are not HTTP —
game servers, databases, custom lab protocols.

**Technology**: Caddy with the `layer4` app (`vuknf/caddy-l4` — the same
image the Docker Hub exit proxy runs).

**Network Path**:
```
Client → knf-hosting.lt:30000-30029 → Router → portforwarder → virtual server
```

**Responsibilities**:
- One `layer4` listener per forward from the `hosting_portforward` table
- Plain TCP passthrough to `hosting-users-dind-{id}:{internal port}` —
  no TLS termination, no HTTP awareness
- The pool (30000-30029, IPv4/TCP only) is published by docker-compose;
  unassigned ports simply refuse connections

**Dynamic Configuration**:
The Caddyfile is regenerated by the Docker Controller from the port
forward table on every change and hot-reloaded — the same
transactional regenerate-and-reload pattern as the users Caddyfile:

```
# Auto-generated for forward: 30005 → VM 42, port 3000
{
    layer4 {
        :30005 {
            route {
                proxy hosting-users-dind-42:3000
            }
        }
    }
}
...
**Known limitation**: the proxy terminates the client's TCP connection,
so the user's app sees the portforwarder's address (172.19.2.6) as the
peer — not the real client IP.
...
| control-docker | users-portforwarder | File | Caddyfile |
| users-caddy | virtual-servers | HTTP | App traffic |
| users-portforwarder | virtual-servers | TCP | Port forwards |
